...
ISR(/*@ unused @*/ USART_RX_vect) {
  ...
  if (fsm_state == (char)0) {
    ...
    /* Convert magnetometer data to strings */
    dtostrf(event.magnetic.x, 6, 6, mag_x);
    dtostrf(event.magnetic.y, 6, 6, mag_y);
    dtostrf(event.magnetic.z, 6, 6, mag_z);
    ...
    strncat(msg, mag_x,     (size_t)maxmsg);
    strncat(msg, mag_y,     (size_t)maxmsg);
    strncat(msg, mag_z,     (size_t)maxmsg);
    strncat(msg, nl,        (size_t)maxmsg);
    strncat(msg, nl,        (size_t)maxmsg);
